...
public class UtilityGenerator {
	 ...
	 * 
	 * @param left
	 *            The number of significant digits on the left side of the
	 *            decimal separator.
	 * @param right
	 *            The number of significant digits (without trailing zeroes) on
	 *            the right side of the separator.
	 * @param decsep
	 *            The String that should be uses for the decimal separator.
	 * 
	 ...
	 * 
	 * @param value
	 *            A long integer representing milliseconds after (or before for
	 ...
	 * 
	 * @param min
	 *            The milliseconds that represent the minimum time for the new
	 *            time.
	 * @param max
	 *            The milliseconds that represent the maximum time for the new
	 *            time.
	 * 
	 ...
	 * @param hasLeadingZeroes
	 *            If true, then leading zeroes will be allowed in the output.
	 ...
	 * 
	 * @param country
	 *            Currently supports "us" or "uk" for formatted results
	 * 
	 ...
	 * 
	 * @param length
	 *            The length of string desired.
	 * @param allowed
	 *            An array of allowable characters.
	 ...
	 * @param delim
	 *            The string to be used to separate the thousands, etc.
	 * @param decsep
	 *            The String that should be uses for the decimal separator.
	 * 
	 ...
	 * 
	 * @param mean
	 *            The mean desired if a series of calls are made to this method.
	 * @param stdev
	 *            The standard deviation desired for a series of calls made to
	 *            this method.
	 * @param invert
	 *            Indicates if sampling from an inverted distribution.
	 * @param add_noise
	 *            Adds a bit of noise to a series of samples
	 * @param min
	 *            Specifies the minimum value desired
	 * @param max
	 *            Specifies the maximum value desired.
	 * 
	 * @return a sampled integer
	 * 
	 * @see #generateRandomGaussianInteger
	 */
	public static int generateRandomGaussianInteger(double mean, double stdev, boolean invert, int add_noise,
	 ...
	 * @see #generateRandomGaussianInteger
	 */
	public static double generateRandomGaussian(double mean, double stdev, boolean invert, int add_noise, double min,
			double max) {
		if (rangen == null) {
			rangen = new Random();
		}
		...
		if (max > min) {
			...
			while (value > max || value < min) {
				value = mean + (stdev * getRandomElement(myGaussian))
						+ (2 * stdev * add_noise * ((rangen.nextInt(5) / 10) + 0.5));
			}
			return value;
		} else if (invert) {
	...
	private static char[] alpha = { 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q',
			'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z' };
	private static char[] alphanumeric = { 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o',
