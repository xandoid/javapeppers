...
public class DbUtil {
	 ...
	 * 
	 * @param rsmd
	 *            The ResultSetMetaData instance that we want to build into map
	 *            of column name -> type
	...
	public static TreeMap<String, Integer> buildTypeScaleTable(
		...
		if (rsmd != null) {
			...
			try {
				...
				for (int i = 1; i <= c; i++) {
					...
					logger.debug("Setting scale for " + name + " to "
									+ scale);
				}
			} catch (SQLException e) {
				logger.error(e.toString());
	...
	public static TreeMap<String, Integer> buildTypeSizeTable(
			ResultSetMetaData rsmd) {
		...
		if (rsmd != null) {
			...
			try {
				...
				for (int i = 1; i <= c; i++) {
					...
					logger.debug("Setting length for " + name + " to "
									+ length);
				}
			} catch (SQLException e) {
				logger.error(e.toString());
	 ...
	 *         names as values
	 */
	public static TreeMap<String, String> buildTypeNameTable(
			ResultSetMetaData rsmd) {
	 ...
	 * @return A java.sql.Timestamp object.
	 */
	public static Timestamp createTimestampYMDHMS(int year, int month, int day,
			int hour, int minute, int second) {
		return new Timestamp(new GregorianCalendar(year, month, day, hour,
	 ...
	 * @return A java.sql.Timestamp object.
	 */
	public static Time createTimeYMDHMS(int year, int month, int day,
			int hour, int minute, int second) {
		return new Time(new GregorianCalendar(year, month, day, hour,
				minute, second).getTimeInMillis());
	}
