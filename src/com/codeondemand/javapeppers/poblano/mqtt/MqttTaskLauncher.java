...
public class MqttTaskLauncher {
	...
	private void buildTaskTable(XdmValue tasks) {
		...
		for (int i = 0; i < foo; i++) {
			...
			XdmSequenceIterator it = task.axisIterator(
					net.sf.saxon.s9api.Axis.ATTRIBUTE, n);
			while (it.hasNext()) {
			...
			while (it.hasNext()) {
				...
				String key = null;
				String value = null;
				XdmSequenceIterator it2 = item.axisIterator(
						net.sf.saxon.s9api.Axis.ATTRIBUTE, n1);
				if (it2.hasNext()) {
	...
	private void buildBrokerTable(XdmValue brokers) {
		...
		for (int i = 0; i < foo; i++) {
			...
			String bip = null;
			QName n = new QName("name");
			XdmSequenceIterator it = broker.axisIterator(
					net.sf.saxon.s9api.Axis.ATTRIBUTE, n);
			while (it.hasNext()) {
				XdmItem item = it.next();
	...
	public void startTask(MqttTaskInfo info) {
		try {
			...
			if (MqttTaskNode.class.isInstance(foo)) {
				...
				logger.error(info.getClassname()
						+ " is not an instance of the MqttTask class\n");
			}
		} catch (Exception e) {
