...
public class TestUtilityGenerator extends TestCase{
	...
	public static void test() {
		...
		assertEquals("Initializing alphnumeric field",true,
				     alphaNumericField.initialize(TokenField.ALPHA_NUMERIC_TOKEN, 16));
		...
		assertEquals("Initializing numeric field",true,
				     numericField.initialize(TokenField.NUMERIC_TOKEN, 10, 2));
		System.out.println( "10 character numeric (US): " + numericField.getNextValue());
		
		TokenField numericField2 = new TokenField("numfield2");
		assertEquals("Initializing numeric field",true,
				     numericField2.initialize(TokenField.NUMERIC_TOKEN, 10, 2,".",","));
		System.out.println( "10 character numeric (UK): " + numericField2.getNextValue());

		TokenField natidfield = new TokenField("natidfield");
		assertEquals("Initializing us nat id  field",true,
				natidfield.initialize(TokenField.NAT_ID_TOKEN ));
	...
	public static void test_generateRandomGaussian(){
		...
		assertNotNull( "Generating gaussian distributed numerics:" ,
	             UtilityGenerator.generateRandomGaussian(mean, stdev, invert, add_noise, min, max));
	}
	...
	public static void test_generateRandomGaussianInteger( ){
		...
		assertNotNull( "Generating gaussian distributed integer:" ,
				             UtilityGenerator.generateRandomGaussianInteger(mean, stdev, invert, add_noise, min, max));
	}
	public static void test_generateRandomAlphaNumeric(){
		assertEquals("Generating a random alphanumeric string",
			      true,
			      UtilityGenerator.generateRandomAlphaNumericString(10).length() == 10);		
	}
	public static void test_generateRandomAlpha(){
		assertEquals("Generating a random alphanumeric string",
			      true,
			      UtilityGenerator.generateRandomAlphaString(10).length() == 10);
	...
	public static void test_generateRandomNumeric(){
		...
		String delim = ",";
		String decpt = ".";
		assertNotNull("Generating a random numeric string",
			      UtilityGenerator.generateRandomNumericString(length,prec,delim,decpt));	
	}
	
	public static void test_generateNationalID( ){
		assertNotNull( "Generating national id for us:",
			       UtilityGenerator.generateNationalIdNumber("us"));
		assertNotNull( "Generating national id for uk:",
			       UtilityGenerator.generateNationalIdNumber("uk"));	
	}
	
	public static void test_generateTimestamp( long millis){
		assertNotNull( "Generating a timestamp:",
			       UtilityGenerator.generateTimeStamp(millis));
	}
