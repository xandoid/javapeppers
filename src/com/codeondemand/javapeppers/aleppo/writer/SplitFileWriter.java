...
public class SplitFileWriter extends DestinationWriter {
        ...
        try {
            ...
            if (record != null && record.toString().length() > 0) {
                if (((RecordCapsule) record).getField("split") != null) {
                    isplit = Integer.parseInt(((RecordCapsule) record).getField("split").getData().toString());
                }
                if (pmap.containsKey("field") && record instanceof RecordCapsule) {
                    ...
                    if (isBinary) {
                        ...
                        wrtr[isplit].write(((RecordCapsule) record).getField(field).getData().toString() + "\n");
                        wrtr[isplit].flush();
                    }
                    ...
                    while (stok.hasMoreElements()) {
                        ...
                        if ((((RecordCapsule) record).getField(tok) != null)) {
                            if (((RecordCapsule) record).getField(tok).getData() != null) {
                                foo.append(((RecordCapsule) record).getField(tok).getData().toString() + "|");
                            }
                        } else {
                            foo.append("|");
                        }
        ...
        if (file != null) {
            ...
            try {
                ...
                if (isBinary) {
                    ...
                    wrtr[split] = new BufferedWriter(new FileWriter(file, append));
                    currentFile[split] = file.getAbsolutePath();
                    logger.debug(AleppoMessages.getString("FileRecordDestination.5") + currentFile[split] //$NON-NLS-1$
     ...
     * @param append   If true, then then if there is an existing file, the new
