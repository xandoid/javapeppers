...
public class DBRecordWriter extends DestinationWriter {
		...
		if (data instanceof RecordCapsule) {
			...
			for (int i = 0; i < rc.getFieldCount(); i++) {
				...
				if (types[i] == java.sql.Types.CHAR || types[i] == java.sql.Types.VARCHAR
						|| types[i] == java.sql.Types.DATE || types[i] == java.sql.Types.TIMESTAMP
						|| types[i] == java.sql.Types.LONGVARCHAR || types[i] == java.sql.Types.OTHER) {
	 ...
	 * 
	 * @param keypfx
	 *            The keypfx value to use. This must be set before any write
