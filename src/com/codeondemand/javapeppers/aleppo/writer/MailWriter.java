...
public class MailWriter extends DestinationWriter {
		...
		if (initialized) {
			try {
				...
				if (props.containsKey("mail.addresses.sendto") && props.containsKey("mail.address.sentfrom")) {
					...
					if (props.containsKey("mail.send_capsule_data")
							&& Boolean.parseBoolean((String) props.get("mail.send_capsule_data"))) {
						// TO-DO: ?
	...
	public boolean doInitialization(RecordCapsule rec) {
		...
		if (rec.getMetaData(AleppoConstants.ALEPPO_DC_MDATA_PROPERTIES_KEY) != null) {
			...
			if (props != null && props.containsKey("mail.smtp.host") && props.containsKey("mail.uid")
					&& props.containsKey("mail.pwd")) {

				agent = new SMTPEmailAgent();
				agent.initialize((String) props.get("mail.smtp.host"), (String) props.get("mail.uid"),
						MiscUtil.decodeB64String((String) props.get("mail.pwd")));
