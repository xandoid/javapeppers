...
public class RecordConnector extends FlowNode implements Runnable {
	...
	public boolean initialize(AbstractRecordSource source, RecordDestination destination) {
		...
		if (source != null && destination != null && src == null && dst.size() == 0) {
			...
			logger.debug(this.toString()
					+ AleppoMessages.getString("RecordConnector.0") //$NON-NLS-1$
					+ src.toString()
					+ AleppoMessages.getString("RecordConnector.1") + destination.toString()); //$NON-NLS-1$
			retval = true;
		} else {
	 ...
	 * 
	 * @param source
	 *            A RecordSource component for getting the records.
	 * @param destination
	 *            A RecordDestination component for putting the records.
	 * @param processor
	 *            An optional RecordProcessor component such as a filter.
	 * 
	 * @return true if successfully initialized.
	 */
	public boolean initialize(RecordSource source, RecordDestination destination,
	 ...
	 * 
	 * @param src
	 *            The record source component.
	 * @param dest
	 *            The record destination component.
	 * @param procs
	 *            An array of processing node components such as filters.
	 * 
	...
	public boolean initialize(RecordSource src, RecordDestination dest, RecordProcessor[] procs) {
		...
		if (procs != null && procs.length > 0) {
			for (int i = 0; i < procs.length; i++) {
				if (procs[i] != null && procs[i] instanceof RecordProcessor) {
					logger
							.debug(AleppoMessages.getString("RecordConnector.2") + procs[i].getClass()); //$NON-NLS-1$
					processors.add((RecordProcessor) procs[i]);
...
//			}
	...
	public boolean doInitialization() {
		...
		if (pmap.containsKey(AleppoConstants.ALEPPO_CONFIG_CONNECTOR_PARAM_DOHEADER)) {
			setDoHeader(Boolean.parseBoolean((String) pmap
					.get(AleppoConstants.ALEPPO_CONFIG_CONNECTOR_PARAM_DOHEADER)));
		}
		...
		try {
			if (pmap.containsKey(AleppoConstants.ALEPPO_CONFIG_ALL_PARAM_MAXROWS)) {
				this.setMaxRows(Long.parseLong((String) pmap
						.get(AleppoConstants.ALEPPO_CONFIG_ALL_PARAM_MAXROWS)));
			}
		...
		try {
			if (pmap.containsKey(AleppoConstants.ALEPPO_CONFIG_ALL_PARAM_MINROWS)) {
				this.setMinRows(Long.parseLong((String) pmap
						.get(AleppoConstants.ALEPPO_CONFIG_ALL_PARAM_MINROWS)));
			}
		} catch (NumberFormatException e) {
			logger.error(AleppoMessages.getString("RecordConnector.18") //$NON-NLS-1$
					+ AleppoConstants.ALEPPO_CONFIG_ALL_PARAM_MAXROWS);
