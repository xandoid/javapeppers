...
public class MultiplexConnector extends Observable implements Observer {
    ...
    public boolean initialize(RecordSource[] srcs, RecordDestination[] dsts, RecordProcessor[] filts) {
    ...
    public boolean initialize(RecordSource[] srcs, RecordDestination dst, RecordProcessor[] filt) {

        boolean retval = true;

        for (RecordSource src : srcs) {
            RecordConnector s2d = createNewConnector();
            retval = retval && s2d.initialize(src, dst, filt);
            Thread t = new Thread(s2d);
            t.start();
        }
        initialized();
    ...
    public boolean initialize(RecordSource src, RecordDestination[] dsts, RecordProcessor[] filt) {
