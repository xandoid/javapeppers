...
public class DataValueTransform extends RecordTransform {
        ...
        if (initialized) {
            ...
            while (it.hasNext()) {
                ...
                if ((dc = input.getField(tag)) != null) {
                    ...
                    if (!dc.isNull()) {
                        if (tables.get(tag).containsKey(dc.getData().toString())) {
                            dc.setData(tables.get(tag).get(dc.getData().toString()));
                        }
                    } else {
    ...
    public boolean doInitialization() {
        ...
        if (pmap.containsKey(AleppoConstants.ALEPPO_CONFIG_ALL_PARAM_FILE) && pmap.containsKey(AleppoConstants.ALEPPO_DELIMITER_KEY)) {
            initialized = loadTranslationTables((String) pmap.get(AleppoConstants.ALEPPO_CONFIG_ALL_PARAM_FILE), (String) pmap.get(AleppoConstants.ALEPPO_DELIMITER_KEY));
            retval = true;
        } else {
    ...
    private boolean loadTranslationTables(String file, String delim) {
        boolean retval = false;
        StreamSource source = new StreamSource(ClassLoader.getSystemResourceAsStream(file));
