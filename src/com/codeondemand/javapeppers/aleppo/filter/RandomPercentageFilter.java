...
public class RandomPercentageFilter extends RecordFilter {
	...
	protected synchronized RecordCapsule filterRecord(RecordCapsule input) {
		...
		if (input != null && initialized) {
			if (rangen != null) {
				double adjust = (objectsPassed - objectsNotPassed) * 2.0
						/ ((objectsPassed + objectsNotPassed) * 1.0);
				if ((filterPct < 90) && rangen.nextDouble() + adjust > filterPct / 100.0) {
					logger
							.debug(AleppoMessages.getString("RandomPercentageFilter.2") + input.toString()); //$NON-NLS-1$
					retval = null;
				} else {
					logger
							.debug(AleppoMessages.getString("RandomPercentageFilter.3") + input.toString()); //$NON-NLS-1$
				}
			}
		}
		return retval;
