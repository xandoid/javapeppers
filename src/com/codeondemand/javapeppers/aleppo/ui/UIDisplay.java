	...
	public boolean initialize(RecordCapsule format) {
		...
		for (int i = 0; i < fieldcount; i++) {
			...
			if (dc.getMetaData("length") != null) {
				int foo = new Integer(dc.getMetaData("length")
						.toString()).intValue();
				foo = Math.min(foo,50);
		...
		if (rc.getMetaData("xlocation") != null
				&& rc.getMetaData("ylocation") != null) {
		...
		if (initialized && !finished) {
			if (record instanceof RecordCapsule) {
				...
				for (int i = 0; i < datafields.length; i++) {
					...
					DataCapsule dc = temp.getField(datafields[i].getName()
							.trim());
