...
public class RecordCapsule extends DataCapsule {
	 ...
	 * 
	 * @param rc2
	 *            The second RecordCapsule to compare with the current
	 *            RecordCapsule
	 ...
	 * 
	 * @param idx
	 *            The positional index of the DataCapsule that is desired.
	 * 
	 ...
	 * 
	 * @param name
	 *            The Name of the DataCapsule to remove.
	 * 
	 ...
	 * 
	 * @param name
	 *            The name of the DataCapsule to fetch.
	 * 
	...
	public DataCapsule getField(String name) {
		DataCapsule retval = null;
		for (int i = 0; i < fields.size(); i++) {
			if (fields.get(i).getName().trim().equals(name.trim())) {
				retval = fields.get(i);
			}
		}
	 ...
	 * 
	 * @param field_name
	 *            The name of the DataCapsule that references the data.
	...
	public boolean isKey(String name) {
		...
		for (int i = 0; i < keys.size(); i++) {
			KeySpecification ks = keys.get(i);
	 ...
	 * 
	 * @param field
	 *            The DataCapsule to add.
	 * @param isKey
	 *            Specifies if this DataCapsule is a key for this RecordCapsule
			...
			if (foo.getData() != null) {
				if (foo.getData() instanceof Object[]) {
					...
					for (int i = 0; i < bar.length; i++) {
						sb.append(separator + bar[i].toString());
						separator = ",";
					}
					sb.append("]");
				} else {
	 ...
	 * 
	 * @param input
	 *            The String containing the tokens for substitution.
	 * 
