 ...
 */
public class ExcelReaderEventModel extends Observable implements HSSFListener,
		Runnable {
		...
		if (slist.contains(new Integer(sheetcount))) {
			...
			case NumberRecord.sid:
				...
				if (numrec.getColumn() < currentRow.length) {
					currentRow[numrec.getColumn()] = new Double(numrec
							.getValue()).toString();
			...
			case LabelSSTRecord.sid:
				...
				if (currentColumn <= currentRow.length - 1) {
					currentRow[lrec.getColumn()] = sstrec.getString(lrec
							.getSSTIndex());
					data_record = true;
