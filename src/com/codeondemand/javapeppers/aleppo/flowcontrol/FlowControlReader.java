...
public class FlowControlReader extends SourceReader {
	...
	public boolean doInitialization() {
		...
		if (props == null && pmap.containsKey(AleppoConstants.ALEPPO_CONFIG_ALL_PARAM_FILE)) {
			props = MiscUtil.loadXMLPropertiesFile((String) pmap
	...
	private void setFlowTime(long flow_id, int cfg_id) {
		try {
			...
			stmt.executeUpdate("Update " + schema + "." + trigger_table
					+ " set (status,start_ts)=('I',current timestamp) where flow_id=" + flow_id + " and cfg_id="
					+ cfg_id);
			c.commit();
		} catch (SQLException e) {
			logger.error(e.toString());
	...
	private ResultSet getFlows() {
		...
		try {
			...
			retval = stmt.executeQuery("Select FLOW_ID,CFG_ID from " + schema + "." + trigger_table
					+ " where STATUS in ('N','E','I')");
	...
	private Connection buildDBConnection() {
		...
		String dbpwd = null;
		if (props.getProperty("db.encrypted.pwd") != null
				&& props.getProperty("db.encrypted.pwd").equals("true")) {
