...
public class AleppoProcess extends RecordProcessor implements Observer {
	...
	public RecordCapsule processRecord(RecordCapsule input) {
		...
		if (process_result) {
			...
			if( process != null){
				logger.debug(AleppoMessages.getString("AleppoProcess.3")
						+ process.getClass().toString() + 
						AleppoMessages.getString("AleppoProcess.4"));				
			}
		} else {
			if( process != null ){
				logger.debug(AleppoMessages.getString("AleppoProcess.5") + //$NON-NLS-1$
						process.getClass().toString()
						+ AleppoMessages.getString("AleppoProcess.6")); //$NON-NLS-1$ 
	...
	public boolean doInitialization() {
		...
		if (pmap.containsKey(AleppoConstants.ALEPPO_CLASS_RUNNABLE_KEY)) {
			runnable_name = (String) pmap
					.get(AleppoConstants.ALEPPO_CLASS_RUNNABLE_KEY);
			try {
	 ...
	 * 
	 * @param o
	 *            The Observable Object which sent the message
	 * @param arg
	...
	public void update(Observable o, Object arg) {
		...
		if (o != null) {
			message = message + o.getClass().toString()
					+ AleppoMessages.getString("AleppoProcess.12"); //$NON-NLS-1$
		}
