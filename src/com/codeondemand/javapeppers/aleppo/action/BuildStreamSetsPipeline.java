...
public class BuildStreamSetsPipeline extends RecordProcessor {
	...
	public RecordCapsule processRecord(RecordCapsule input) {
		...
		if (input.checkField("ss_output_file_name")) {
			input.getField("ss_output_file_name")
					.setData(new String(dbout + "_" + input.getField("schema").getData().toString().toLowerCase() + "_"
							+ input.getField("table").getData().toString().toLowerCase()));
		} else {
			DataCapsule dc = new DataCapsule("ss_output_file_name",
					new String(dbout + "_" + input.getField("SCHEMA").getData().toString().toLowerCase() + "_"
							+ input.getField("TABLE_NAME").getData().toString().toLowerCase() + ".json"));
	...
	private String subTokens(String foo, RecordCapsule rc) {
		...
		if (retval.contains("@PIPE_DESC@")) {
			retval = retval.replace("@PIPE_DESC@",
					"Auto generated pipeline from @SCHEMA@.@TABLE@ to HDFS/Hive " + MiscUtil.getCurrentTimeString());
		}
		...
		if (retval.contains("@HDFS_TABLE@")) {
			retval = retval.replace("@HDFS_TABLE@",
					"t_" + dbout + "_" + rc.getField("SCHEMA").getData().toString().toLowerCase() + "_e_"
							+ rc.getField("TABLE_NAME").getData().toString().toLowerCase());
		}
	...
	private String genQuery(RecordCapsule rc, boolean cast) {
		...
		if (pmap.containsKey("incl_tbl")) {
			...
			if (incl_tbl) {
				tbl_select_token = new String(
						tok_delim + "'" + rc.getField("TABLE_NAME").getData().toString() + "' as TBL_NAME");
			}
		}
		...
		for (int i = 0; i < cnt; i++) {
			...
			if (cast) {
				if (f.getField(i).getMetaData("typeName").equals("RAW")) {
					...
					sb.append(  " cast(\\\"" + f.getField(i).getName() + "\\\" as VARCHAR(35)) as \\\""
							+ f.getField(i).getName()+"\\\"");
				} else if (f.getField(i).getMetaData("typeName").equals("BLOB")) {
					sb.append("utl_raw.cast_to_varchar2(dbms_lob.substr(\\\"" + f.getField(i).getName()
							+ "\\\" ,2000,0)) as \\\"" + f.getField(i).getName()+"\\\"");
