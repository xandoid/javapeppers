...
public class FieldFrequency extends MonitorProcess {
        ...
        if (br != null) {
            ...
            try {
                ...
                for (int i = 0; i < names.size(); i++) {
                    ...
                    br.write(names.get(i) + "|" + counters.get(i) + "|" + nf.format(pct / 100.0) + "|");
                    String min = "null";
    ...
    protected synchronized boolean doMonitor(RecordCapsule input) {
        ...
        if (counters == null) {
            ...
            for (int i = 0; i < input.getFieldCount(); i++) {
                ...
                if (input.getField(i).isNull() || input.getField(i).getData() == null || input.getField(i).getData().toString().length() == 0) {
                    counters.add(0L);
                } else {
                    counters.add(1L);
                    doMinMax(name, dc);
            ...
            for (int i = 0; i < input.getFieldCount(); i++) {
                ...
                if (!dc.isNull() && !input.getField(i).isNull() && input.getField(i).getData() != null && input.getField(i).getData().toString().length() != 0) {
                    counters.set(i, counters.get(i) + 1);
                    maxcount = maxcount > counters.get(i) ? maxcount : counters.get(i);
                }
                if (!dc.isNull()) {
                    doMinMax(name, dc);
                    doPosNeg(name, dc);
    ...
    private void doMinMax(String name, DataCapsule dc) {
        if (dc.getData() != null && dc.getData().toString().length() > 0) {
            ...
            if (dc.getData() instanceof Number && minmap.get(name) instanceof Number) {
                Double foo = new Double(dc.getData().toString());
                Double bar = new Double(minmap.get(name).toString());
                if (foo <= bar) {
                    minmap.put(name, foo);
                }
            } else if (dc.getData().toString().compareTo(minmap.get(name).toString()) <= 0) {
                minmap.put(name, dc.getData());
            }

            if (dc.getData() instanceof Number && maxmap.get(name) instanceof Number) {
                Double foo = new Double(dc.getData().toString());
                Double bar = new Double(maxmap.get(name).toString());
                if (foo > bar) {
                    maxmap.put(name, foo);
                }
            } else if (dc.getData().toString().compareTo(maxmap.get(name).toString()) > 0) {
                maxmap.put(name, dc.getData());
            }
        }
    }
