...
public class FlowCountMonitor extends MonitorProcess {
	...
	public void done() {
		...
		if (decimals) {
			System.out.println(name+":"+new Double((getRecordCount() + 1)
					/ elapsed_time_sec)
					+ ":" + (getRecordCount() + 1));

		} else {
			System.out.println(new Double((getRecordCount() + 1)
					/ elapsed_time_sec).longValue()
					+ ":" + (getRecordCount() + 1));
	...
	protected boolean doMonitor(RecordCapsule input) {
		if (!initialized) {
			...
			if (pmap.containsKey("show_time")) {
				show_time = Boolean
						.parseBoolean((String) pmap.get("show_time"));
			}
			if (pmap.containsKey("do_notify")) {
				do_notify = Boolean
						.parseBoolean((String) pmap.get("do_notify"));
			}
		...
		if (counter == interval) {
			...
			Double intervalPerSec = new Double(interval)
					/ new Double(interval_time_sec);
			Double totPerSec = new Double(cnt) / new Double(tot_time_sec);
			if (decimals) {
				showCount(name, cnt, String.valueOf(totPerSec),
						String.valueOf(intervalPerSec));
			} else {
				showCount(name, cnt, totPerSec.longValue(),
						intervalPerSec.longValue());
			}
			counter = 0L;
	...
	protected void showCount(String name, Long cnt, Long totPerSec,
			Long intervalPerSec) {
		if (show_time) {
			String ts = MiscUtil.getCurrentTimeString();
			System.out.println(ts + ":" + name + ":" + totPerSec + ":"
					+ intervalPerSec + ":" + cnt);
		} else {
			System.out.println(name + ":" + totPerSec + ":" + intervalPerSec
					+ ":" + cnt);
		}
	}

	protected void showCount(String name, Long cnt, String totPerSec,
			String intervalPerSec) {
		if (show_time) {
			String ts = MiscUtil.getCurrentTimeString();
			System.out.println(ts + ":" + name + ":" + totPerSec + ":"
					+ intervalPerSec + ":" + cnt);
