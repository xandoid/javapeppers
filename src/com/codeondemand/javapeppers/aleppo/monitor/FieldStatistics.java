...
public class FieldStatistics extends MonitorProcess {
		...
		for (int i = 0; i < rc.getFieldCount(); i++) {
			...
			if (stats.containsKey(dc.getName()) && !dc.isNull()) {
				if (stats.get(dc.getName()) instanceof SummaryStatistics) {
					((SummaryStatistics) stats.get(dc.getName())).addValue(new Double(dc.getData()
							.toString()).doubleValue());
				} else {
					((DescriptiveStatistics) stats.get(dc.getName())).addValue(new Double(dc
							.getData().toString()).doubleValue());
				}
			}
		}
		return true;
	...
	public boolean initialize(RecordCapsule rc) {
		for (int i = 0; i < rc.getFieldCount(); i++) {
			...
			if (dc.getMetaData("stat_type") != null
					&& dc.getMetaData("stat_type").toString().equalsIgnoreCase("summary")) {
