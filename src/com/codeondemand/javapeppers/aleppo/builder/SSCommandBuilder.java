...
public class SSCommandBuilder extends NullBuilder {
	 ...
	 * 
	 * @param rc
	 *            The RecordCapsule containing the data structure information.
	 * 
	...
	public Object buildRecord(RecordCapsule rc) {
		...
		if (rc.checkField("import_output_file_name")) {
			rc.getField("import_output_file_name")
					.setData(new String("import_"+rc.getField("SCHEMA").getData().toString().toLowerCase() + "_"
							+ rc.getField("TABLE_NAME").getData().toString().toLowerCase() + ".sh"));
		} else {
			DataCapsule dc = new DataCapsule("import_output_file_name",
					new String("import_"+rc.getField("SCHEMA").getData().toString().toLowerCase() + "_"
							+ rc.getField("TABLE_NAME").getData().toString().toLowerCase() + ".sh"));
			rc.addDataCapsule(dc, false);
		}
		if (rc.checkField("delete_output_file_name")) {
			rc.getField("delete_output_file_name")
					.setData(new String("delete_"+rc.getField("SCHEMA").getData().toString().toLowerCase() + "_"
							+ rc.getField("TABLE_NAME").getData().toString().toLowerCase() + ".sh"));
		} else {
			DataCapsule dc = new DataCapsule("delete_output_file_name",
					new String("delete_"+rc.getField("SCHEMA").getData().toString().toLowerCase() + "_"
							+ rc.getField("TABLE_NAME").getData().toString().toLowerCase() + ".sh"));
			rc.addDataCapsule(dc, false);
		}

		if (rc.checkField("reset_output_file_name")) {
			rc.getField("reset_output_file_name")
					.setData(new String("reset_"+rc.getField("SCHEMA").getData().toString().toLowerCase() + "_"
							+ rc.getField("TABLE_NAME").getData().toString().toLowerCase() + ".sh"));
		} else {
			DataCapsule dc = new DataCapsule("reset_output_file_name",
					new String("reset_"+rc.getField("SCHEMA").getData().toString().toLowerCase() + "_"
							+ rc.getField("TABLE_NAME").getData().toString().toLowerCase() + ".sh"));
			rc.addDataCapsule(dc, false);
		}
		if (rc.checkField("start_output_file_name")) {
			rc.getField("start_output_file_name")
					.setData(new String("start_"+rc.getField("SCHEMA").getData().toString().toLowerCase() + "_"
							+ rc.getField("TABLE_NAME").getData().toString().toLowerCase() + ".sh"));
		} else {
			DataCapsule dc = new DataCapsule("start_output_file_name",
					new String("start_"+rc.getField("SCHEMA").getData().toString().toLowerCase() + "_"
							+ rc.getField("TABLE_NAME").getData().toString().toLowerCase() + ".sh"));
			rc.addDataCapsule(dc, false);
		}
		if (rc.checkField("status_output_file_name")) {
			rc.getField("status_output_file_name")
					.setData(new String("status_"+rc.getField("SCHEMA").getData().toString().toLowerCase() + "_"
							+ rc.getField("TABLE_NAME").getData().toString().toLowerCase() + ".sh"));
		} else {
			DataCapsule dc = new DataCapsule("status_output_file_name",
					new String("status_"+rc.getField("SCHEMA").getData().toString().toLowerCase() + "_"
							+ rc.getField("TABLE_NAME").getData().toString().toLowerCase() + ".sh"));
			rc.addDataCapsule(dc, false);
		}
		...
		if (rc.checkField("stop_output_file_name")) {
			rc.getField("stop_output_file_name")
					.setData(new String("stop_"+rc.getField("SCHEMA").getData().toString().toLowerCase() + "_"
							+ rc.getField("TABLE_NAME").getData().toString().toLowerCase() + ".sh"));
		} else {
			DataCapsule dc = new DataCapsule("stop_output_file_name",
					new String("stop_"+rc.getField("SCHEMA").getData().toString().toLowerCase() + "_"
							+ rc.getField("TABLE_NAME").getData().toString().toLowerCase() + ".sh"));
			rc.addDataCapsule(dc, false);
		}
	...
	private String buildImportCommand() {
		...
		sb.append("streamsets cli -u $bar -p $foo -U $ssurl store import -n "+ dbout +"_"+
		           schema+"_"+table+" -f ./"+dbout+"_"+schema+"_"+table+".json \n");
		sb.append("");
