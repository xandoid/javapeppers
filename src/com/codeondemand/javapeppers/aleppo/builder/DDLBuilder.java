...
public class DDLBuilder extends NullBuilder {
	...
	public boolean doInitialization() {
		boolean retval = true;
		if (pmap.containsKey(AleppoConstants.ALEPPO_DB_SCHEMA_KEY)
				&& pmap.containsKey(AleppoConstants.ALEPPO_DB_TABLE_KEY)) {
			table = (String) pmap.get(AleppoConstants.ALEPPO_DB_TABLE_KEY);
			schema = (String) pmap.get(AleppoConstants.ALEPPO_DB_SCHEMA_KEY);
		} else {
			logger.debug("No 'schema' and/or 'table' attributes provided in the)" +
					" configuration file.");
		}
		return retval;
	}
				...
				if (dc != null) {
					...
					if (dc.getMetaData("typeName") != null && dc.getMetaData("length") != null) {
						sb
								.append(addColumn(
										dc.getName(),
										dc.getMetaData("typeName").toString(), //$NON-NLS-1$
