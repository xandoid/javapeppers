...
public class HiveTableDefinitionBuilder extends NullBuilder {
	...
	public Object buildRecord(RecordCapsule rc) {
		...
		if (rc.checkField("ddl_output_file_name")) {
			rc.getField("ddl_output_file_name")
					.setData(new String(rc.getField("SCHEMA").getData().toString().toLowerCase() + "_"
							+ rc.getField("TABLE_NAME").getData().toString().toLowerCase() + "_definition.hql"));
		} else {
			DataCapsule dc = new DataCapsule("ddl_output_file_name",
					new String(rc.getField("SCHEMA").getData().toString().toLowerCase() + "_"
							+ rc.getField("TABLE_NAME").getData().toString().toLowerCase() + ".hql"));
			...
			for (int i = 0; i < fc.getFieldCount(); i++) {
				...
				if (dc != null) {
					...
					if (dc.getMetaData("typeName") != null && dc.getMetaData("length") != null) {
						sb.append(addColumn(dc.getName(),
								xlate(dc.getMetaData("typeName").toString(), dc.getMetaData("length").toString(),
										dc.getMetaData("scale").toString()), //$NON-NLS-1$
								Integer.parseInt((dc.getMetaData("length").toString())),
								Integer.parseInt((dc.getMetaData("scale").toString())), isNullable)); //$NON-NLS-1$
					}
				}
				colnum++;
			}
	...
	private String buildStart() {
		...
		sb.append("use "+dbout +";\n");
		sb.append("--DROP TABLE IF EXISTS " + mapTableName(tblname) + ";\n");		
		sb.append("CREATE EXTERNAL TABLE IF NOT EXISTS " + mapTableName(tblname) + 
				"( \n\tingest_id string, \n\trid_orig string, \n\tingest_ts string,"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
